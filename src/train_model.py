...
from training import DataLoaderLite, CosineDecayLR, train
...
batch_size, B, T = 32768, 4, 1024
assert batch_size % (B * T) == 0, "Batch size is not divisible by (B * T)"
grad_accum_steps = batch_size // (B * T)
train_loader = DataLoaderLite(device, B, T, tok, "./data/tiny_shakespeare.txt")
...
optimizer = model.configure_optimizer(device, 0.1, 6e-3)
scheduler = CosineDecayLR(optimizer, 3e-3, 0.1 * 3e-4, 50, 450)
train(
    device, model, train_loader, 
    optimizer, scheduler, 
    450, grad_accum_steps
)
